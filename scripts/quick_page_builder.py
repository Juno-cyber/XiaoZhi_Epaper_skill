...
def build_page(ip, page, layout_text):
    ...
    last_add_args = None  # 记住最后一个元素，供自定义页面 refresh 重放
    ...
    for line in lines:
        ...
        if cmd == "clear":
            ...
            elif last_add_args is not None:
                # 自定义页面没有独立刷新工具；重放最后一个元素（同 id 幂等替换）并带 refresh=true
                replay = dict(last_add_args)
                replay["refresh"] = True
                r = call_mcp(ip, "fridge.page.element.control", replay)
                print(f"  [refresh] (replay last element) -> {r.get('status', '?')}")
            else:
                print("  [refresh] skip: no elements added yet")
            time.sleep(0.3)
